...
class sliderRuler {
  ...
  dreawCanvas() {
    ...
    for (let i = Math.round(startValue / precision * 10) / 10; i <= endValue / precision; i++) {
      ...
      // console.log(origin.x + (i - startValue / precision) * divide)
      // 画线到刻度高度，10的位数就加高
      context.lineTo(origin.x + (i - startValue / precision) * divide, i % 10 === 0 ? canvasHeight*2-heightDecimal+20 : canvasHeight*2-heightDigit+8);
