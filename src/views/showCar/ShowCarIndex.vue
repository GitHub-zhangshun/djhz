...
import showCarDetail from '@/components/showCar'
    export default {
        ...
        methods:{
            // init() {
            //     this.$axios('center_order_cartgoodsbypage',{index: this.pageNumber,size:4})
            //     .then( res => {
            ...
            onLoad() {    
                ...
                // 异步更新数据
                this.$axios('center_order_cartgoodsbypage',{index: this.pageNumber,size:20})
                .then( res => {  
                    Toast.clear()                  
                    if(res.result.length == 0 && this.pageNumber==1){
                        this.show = 2
                        return
                    }
                    if(this.pageNumber == 1){
                    ...
                    this.loading = false;
                    this.pageNumber++;
                    if(this.carDetails.length >= this.totalPage){
                        this.finished = true
                    }
                })
                .catch( () => {})
