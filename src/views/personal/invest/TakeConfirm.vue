<template>
<!-- 提金确认 -->
    <div class="take_confirm bg_base">
        <!-- 页头 -->
        ...
        <div class="content">
            ...
            <!-- 购买商品列表 -->
            <div class="product_list bg_white">
                <div v-for="(item,index) in ticketInfo.orderList" :key="index">
            ...
            <div class="invoice bg_white text_size_28">
                ...
                <div class="row" v-if="label =='gostore'" @click="chooseTakeDate">
                    <p>预约提金时间</p>
                    <div class="row">
                        <span class="color_gray"><label v-if="selectedDate">{{ selectedDate }}</label><label v-else>选择提金时间</label></span>
                        <img src="@/assets/images/ticket/path.png" >
                    </div>
                </div>
            ...
            <!-- 使用账户余额抵扣 -->
            <div class="deduction row bg_white text_size_28" v-if="label == 'delivery'">
                <p>使用账户余额抵扣<span class="color_gold">&nbsp;&yen;{{ Number(price)>=Number(needPayPrice)?ticketInfo.freight:price || '0.00' }}&nbsp;</span>元</p>
                <van-switch
                v-model="checked"
                active-color="#f5ab0b"
        ...
        <div class="footer row text_size_28 text_center">
            <div class="price" v-if="label == 'delivery'">
                运费：<span class="color_gold text_size_30">&yen;{{ ticketInfo.freight || '0.00' }}</span>
            </div>
            ...
            <van-button v-if="label == 'gostore'" :disabled="hasStore?false:true"  @click="submit" :loading="isLoading" loading-text="提交中..." class="store_full">提交订单</van-button>
        </div>
...
import ToastPop from '@/components/Toast'
    ...
    export default {
        ...
        watch:{
            checked(val){
                if(val){
                    ...
                    this.needPayPrice = this.ticketInfo.freight
                }
            }
        },
        methods: {
            ...
            submit(){
                ...
                this.$axios(
                    'center_order_goldorderextractgoods',
                    { orderType:this.$route.query.type, orderIds:[this.$route.query.id], isDelivery:isdelivery, isBalanceDeduction:this.checked, addressId:this.addressId, storeId:this.storeId, extractTime:this.selectedDate, isPersonal:isPersonal, title:title, taxNumber:taxNumber, sourceSimplify:'order_source_h5', remark:this.message } 
                ).then(data => {
                        ...
                        if(isdelivery){
                            ...
                            if(!data.result.isBankCardPay){
                                this.$store.commit({
                                    ...
                                    showToast: true,
                                    hintType:4,
                                    needpaymoney:this.ticketInfo.freight
                                });
                            }else{
                                //选择银行卡
                            ...
                            this.$router.replace({path:'/personal/success', query:{type:'take', id: data.result.orderId, code: data.result.extractCode }})
                        }
        ...
        created(){    
            ...
            //获取商品信息
            this.$axios('center_order_extractgoodsconfirm', {type: this.$route.query.type, orderIds:[this.$route.query.id]})
            .then(data => {
                // console.log(data)
                ...
                this.ticketInfo.orderList.forEach(item => {
