                ...
                >
                    ...
                    <div class="main" @click="toDetail(item.id)"> 
                        <div class="detail text_size_28">
                            <p>提金克重合计：{{ item.orderWeight || 0}}克</p>
                            <p>实际购买金额合计：{{ item.orderMoney || '0.00' }}元</p>
                        </div>
                    </div>
                    <div class="bot row">
                        <div class="text_size_28">
                            <p v-if="item.orderStatus !== '待自提' && item.isDelivery">运费：{{ item.freight || '0.00' }}元</p>
                            <p v-if="(item.orderStatus == '待自提' || item.orderStatus == '已完成') && !item.isDelivery">自提码：{{ item.code }}</p>
                        </div>
                        <div class="btn">   
                            ...
                            <div v-if="item.orderStatus == '待收货'">
                                <van-button round class="normal" @click="$router.push({path:'/personal/delivery', query:{id:item.id, type:'take'}})">查看物流</van-button>
                                <van-button round class="another" @click="confirmReceived(item.id)">确认收货</van-button>
                            </div>
                            <van-button round class="normal" v-if="item.orderStatus == '待自提'" @click="$router.push('/chat')">联系客服</van-button> 
                        </div>
                    </div>
                    </van-cell>
                </van-list>
    ...
    export default {        
        ...
        methods: {
            ...
            onLoad() {
                ...
                //请求订单
                this.$axios('order_goldorder_golddeliveryorderlistfrontbypage', obj)
                .then(data => {
                    Toast.clear()
                    this.pullRefresh = false
                    this.loading = false;
                    ...
                    data.result && data.result.forEach(item => {
                        item.frontStatus = findState(item.frontStatus)
                        switch(item.frontStatus){
                            case 'front_gold_delivery_unsend' : item.orderStatus = '待发货'; break;
                            case 'front_gold_delivery_waitgoods' : item.orderStatus = '待收货'; break;
                            case 'front_gold_delivery_extract' : item.orderStatus = '待自提'; break;
                            case 'front_gold_delivery_finish' : item.orderStatus = '已完成'; break;
                        }

                        item.orderMoney = item.orderMoney && item.orderMoney.toFixed(2)
                        this.list.push(item)
                    })
            ...
            //跳转订单详情
            toDetail(id){
                this.$router.push({path:'/personal/invest/taketicketdetail', query:{id:id}})
            },
            ...
            confirmReceived(id){
                ...
                message: '您确认收到货了吗？'
                    ...
                    this.$axios('center_order_goldorderreceivedgoods', id)
                    .then(data => {
                        // if(data.code != 0){
        ...
        created(){
            //获取状态列表
            this.$axios('dic_face_getlistdetailrtbyindexsimplify', 'front_gold_delivery_status')
            .then(data => {
